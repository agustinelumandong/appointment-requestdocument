...
if (event.target.classList.contains('check-availability-btn')) {
...
const img = new Image();
img.src = '/uploads/images/LGU%20LOGO.png'; // Use encoded space
img.style.width = '200px';

// Open popup and insert image
const popup = window.open('', '', 'width=300,height=300');
if (popup) {
  popup.document.write(`<p>You are logged in as <strong>${userRole}</strong></p>`);
            ...
            success: function(response) {
                if (response.success && response.services) {
                    ...
                    services.forEach((service, index) => {
                        ...
                        const allowedSlugs = ['request-document', 'request-document2', 'request-document3', 'request-document4', 'request-document5']; // add more slugs here
